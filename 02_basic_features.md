...
在前一节中已经提及，R 的核心是命令行界面的命令解释器。而在 RStudio 中，Console 窗口是四个窗口中唯一一个没有标签页的。也就是说，无论如何，你总会与 Console 打交道。

RStudio Console 包含了多种功能，目的是为了使与 R 的交互更直接和高效。本节将回顾这些功能。学习使用这些，并辅以相关的 Source 和 History 窗口中的功能，可以迅速带来极大的收获，提升你的整体生产力。
...
### 管理多个文件
而 Source 窗口就是专门被设计来编辑长段代码的。其上方的标签页设计，可以使得你同时打开多个文件，并在这些文件之间切换。如果文件过多，还可以搜索特定的文件名，见下图：
